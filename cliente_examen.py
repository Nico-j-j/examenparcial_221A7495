...
class Cliente():
    ...
    def sec_mult(self, A, B):
        print("Multiplicación Secuencial:")
    ...
    def recibir(self):
        ...
        while True:
            try:
                ...
                A = [[random.randint(0, 10) for i in range(data1[1])]
                     for j in range(data1[0])]
                # Genero B[6][21535220]con num. aleatorios del 0 al 215, ver excel
                B = [[random.randint(0, 10) for i in range(data1[2])]
                     for j in range(data1[1])]
                print(A)
                print(B)

                inicioS = time.time()
                result_secuencial = self.sec_mult(A, B)
                print(result_secuencial)
                finS = time.time()
                #inicioP = time.time()
                #result_paralelo = self.par_mult(A, B)
                #print(result_paralelo)
                #finP = time.time()
                n_fil_A = len(A)  
                n_col_A = len(A[0])  
                n_fil_B = len(B)  
                ...
                sizeA= str(n_fil_A) + ","+ str(n_col_A)
                sizeB= str(n_fil_B) + ","+ str(n_col_B)
                print('\n\nMatriz  A y B se han multiplicado con exito en SECUENCIAL ha tardado ', str(finS-inicioS))
                print("El tamaño de A es: ", sizeA)
                print("El tamaño de B es: ", sizeB)
